...
class Chip8 {
  BYTE gameMemory[0x1000]; // 0x0 to 0xFFF <=> 4096 bytes of memory
  ...
  BYTE stackPointer = -1;  // Self Explanatory jimmy
  BYTE screenData[32][64]; // 64x32 resolution display
  BYTE delayTimer;
  BYTE soundTimer;
  BYTE keypad[16];
  WORD opcode;
...
public:
  ...
  void cycle() {
    WORD opcode = gameMemory[programCounter];
    opcode <<= 8;                             // LB to UB
